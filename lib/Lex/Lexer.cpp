...
Lexer::Lexer(const MemoryBuffer *InputBuffer, LangFeatures features) :
  : Buffer(InputBuffer), KeepComments(false), TokStart(0),
  IsAtStartOfLine(true), Features(features) {
  InitCharacterInfo();

  TokStart = InputFile->getBufferStart();
}
...
static bool isHexaLiteral(const char *Start, const LangFeatures &Features) {
  unsigned Size;
...
void Lexer::LexNumericConstant(Token &Result, const char *CurPtr) {
  ...
  if ((C == '-' || C == '+') && (PrevCh == 'E' || PrevCh == 'e')) {
    ...
    if (!isHexaLiteral(BufferPtr, Features))
  ...
  FormTokenWithChars(Result, CurPtr, tok::number);
}
